...
public class FirstPersonControllerEdited : MonoBehaviour
{
    ...
    public bool isSprinting = false; //changed to public
    ...
    private bool dodgeRequested = false;
    ...
    private void Update()
    {
        ...
        #region Dodge Stamina Regen
        if (enableDodge)
        {
            // Slowly regain stamina when not dodging
            if (!isSprinting) 
            {
            ...
            if (useSprintBar)
            {
                ...
                if (hideBarWhenFull)
                {
                    sprintBarCG.alpha = (sprintRemaining < sprintDuration) ? 1 : Mathf.MoveTowards(sprintBarCG.alpha, 0, Time.deltaTime);
                }
            }
            
        }
        if (enableDodge && Input.GetKeyDown(dodgeKey) && sprintRemaining >= dodgeStaminaCost && !isSprintCooldown)
        {
            dodgeRequested = true; 
        }
        #endregion
    ...
    void FixedUpdate()
    {
        ...
        if (playerCanMove)
        {
            ...
            Vector3 targetVelocity = transform.TransformDirection(inputVector) * walkSpeed;
            ...
            if (dodgeRequested)
            {
                ExecuteDodge(new Vector3(Input.GetAxis("Horizontal"), 0, Input.GetAxis("Vertical")));
                dodgeRequested = false; // Reset it so you don't double-dodge
            }
            ...
            if (enableDodge && Input.GetKeyDown(dodgeKey) && sprintRemaining >= dodgeStaminaCost && !isSprintCooldown)
            {
                ExecuteDodge(inputVector);
            }
        }
        #endregion
    }
    #region Execute Dodge

    private void ExecuteDodge(Vector3 movementDir)
    {
        // 1. Determine Direction (Default to forward if standing still)
        Vector3 dodgeDir = movementDir == Vector3.zero ? transform.forward : transform.TransformDirection(movementDir).normalized;

        // 2. Clear current velocity (This makes it "Instant" and snappy)
        // Without this line, your current walking momentum fights the dodge
        rb.velocity = new Vector3(0, rb.velocity.y, 0); 

        // 3. Apply the Burst
        // Use Impulse for an immediate change in momentum
        rb.AddForce(dodgeDir * dodgeForce, ForceMode.Impulse);
        
        // 4. Pay the Stamina Cost
        sprintRemaining -= dodgeStaminaCost;

        // 5. Start Visuals
        StartCoroutine(DodgeVisualEffect());
    ...
    private IEnumerator DodgeVisualEffect()
    {
        isSprinting = true; // Reusing this bool to trigger your existing FOV/Headbob logic
...
// Custom Editor
#if UNITY_EDITOR
    [CustomEditor(typeof(FirstPersonControllerEdited)), InitializeOnLoadAttribute]
    public class FirstPersonControllerEditedEditor : Editor
    {
    FirstPersonControllerEdited fpc;
    SerializedObject SerFPC;

    private void OnEnable()
    {
        fpc = (FirstPersonControllerEdited)target;
